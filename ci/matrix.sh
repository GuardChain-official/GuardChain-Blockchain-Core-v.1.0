...
export BUILDER_IMAGE_NAME="guardchain-builder-$BUILD_TARGET-$JOB_NUMBER"
...
if [ "$BUILD_TARGET" = "arm-linux" ]; then
  ...
  export USE_SHELL="/bin/guardchain"
  export PYZMQ=true
  export RUN_TESTS=true
  ...
  export CPPFLAGS="-DDEBUG_LOCKORDER -DENABLE_GUAC_DEBUG"
  export PYZMQ=true
  export RUN_TESTS=true
