...
# guardchain_hash
RUN git clone https://github.com/guardchainpay/guardchain_hash
RUN cd guardchain_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} guardchain
RUN useradd -u ${USER_ID} -g guardchain -s /bin/bash -m -d /guardchain guardchain
...
RUN mkdir /guardchain-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /guardchain-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /guardchain-src
