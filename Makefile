...
pkgdatadir = $(datadir)/guardchaincore
pkgincludedir = $(includedir)/guardchaincore
pkglibdir = $(libdir)/guardchaincore
pkglibexecdir = $(libexecdir)/guardchaincore
...
CONFIG_HEADER = $(top_builddir)/src/config/guardchain-config.h
CONFIG_CLEAN_FILES = libguardchainconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libguardchainconsensus.pc.in \
	...
	$(top_srcdir)/src/config/guardchain-config.h.in \
...
ACLOCAL = ${SHELL} /home/u18/c/guardfinal/GuardChain-SRC/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /home/u18/c/guardfinal/GuardChain-SRC/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /home/u18/c/guardfinal/GuardChain-SRC/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /home/u18/c/guardfinal/GuardChain-SRC/build-aux/missing automake-1.15
AWK = mawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = guardchain-cli
BITCOIN_DAEMON_NAME = guardchaind
BITCOIN_GUI_NAME = guardchain-qt
BITCOIN_TX_NAME = guardchain-tx
...
COPYRIGHT_HOLDERS_FINAL = The GuardChain Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = GuardChain Core
COPYRIGHT_YEAR = 2020
CPP = gcc -E
...
MAINT = 
MAKEINFO = ${SHELL} /home/u18/c/guardfinal/GuardChain-SRC/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL = 
OTOOL64 = 
PACKAGE = guardchaincore
PACKAGE_BUGREPORT = https://github.com/guardchainpay/guardchain/issues
PACKAGE_NAME = GuardChain Core
PACKAGE_STRING = GuardChain Core 1.0.0
PACKAGE_TARNAME = guardchaincore
PACKAGE_URL = https://guardchain.org/
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /home/u18/c/guardfinal/GuardChain-SRC
abs_srcdir = /home/u18/c/guardfinal/GuardChain-SRC
abs_top_builddir = /home/u18/c/guardfinal/GuardChain-SRC
abs_top_srcdir = /home/u18/c/guardfinal/GuardChain-SRC
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /home/u18/c/guardfinal/GuardChain-SRC/build-aux/install-sh
...
pkgconfig_DATA = libguardchainconsensus.pc
...
DIST_CONTRIB = $(top_srcdir)/contrib/guardchain-cli.bash-completion \
	       $(top_srcdir)/contrib/guardchain-tx.bash-completion \
	       $(top_srcdir)/contrib/guardchaind.bash-completion \
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info \
  leveldb_baseline.info test_guardchain_filtered.info total_coverage.info \
  baseline_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_guardchain_coverage.info test_guardchain.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/GuardChain-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/guardchain-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/guardchain-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/guardchain-config.h
$(top_srcdir)/src/config/guardchain-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/guardchain-config.h src/config/stamp-h1
libguardchainconsensus.pc: $(top_builddir)/config.status $(srcdir)/libguardchainconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/GuardChain-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/GuardChain-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/GuardChain-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_guardchain.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_guardchain -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_guardchain_filtered.info: test_guardchain.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#rpc_test.info: test_guardchain_filtered.info
...
#test_guardchain_coverage.info: baseline_filtered_combined.info test_guardchain_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_guardchain_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_guardchain_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_guardchain_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_guardchain.coverage/.dirstamp:  test_guardchain_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_guardchain.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf coverage_percent.txt test_guardchain.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
