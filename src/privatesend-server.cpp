...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all GuardChain related functionality
...
/*
    ...
    allow endless transaction that would bloat GuardChain and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all GuardChain specific functionality
