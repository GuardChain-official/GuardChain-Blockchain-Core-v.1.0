...
const char * const BITCOIN_CONF_FILENAME = "guardchain.conf";
const char * const BITCOIN_PID_FILENAME = "guardchaind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "guardchain" is a composite category enabling all GuardChain-related debug output
                if(ptrCategory->count(std::string("guardchain"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GuardChain
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GuardChain
    // Mac: ~/Library/Application Support/GuardChain
    // Unix: ~/.guardchain
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GuardChain";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GuardChain";
#else
    // Unix
    return pathRet / ".guardchain";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty guardchain.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override guardchain.conf
