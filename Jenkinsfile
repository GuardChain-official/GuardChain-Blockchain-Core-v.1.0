...
// which are part of the GuardChain repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="guardchain-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /guardchain-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "guardchainpay-guardchain/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from guardchainpay-guardchain/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'guardchainpay-guardchain/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from guardchainpay-guardchain/develop"
            }
          }

          if (hasCache) {
            sh "cd /guardchain-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /guardchain-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /guardchain-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /guardchain-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /guardchain-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /guardchain-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /guardchain-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
